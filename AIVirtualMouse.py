import cv2
import numpy as np
import HandTrackingModule as htm
import time
import autopy

# 1. Find landmarks
# 2. Get the tip of the middle and index finger
# 3. Check which fingers are up
# 4. Only index finger :Moving mode
# 5. Convert coordinates
# 6. Smoothen the values
# 7. Move mouse
# 8. Both index and middle fingers are up : Clicking mode
# 9. Find distance between fingers
# 10. Click mouse if distance short
# 11. Frame rate
# 12. Display

wCam, hCam = 640, 480

cap = cv2.VideoCapture(0)
cap.set(3, wCam)
cap.set(4, hCam)
pTime=0
plocX, plocY = 0, 0
clocX, clocY = 0, 0

detector = htm.handDetector(maxHands=1)  # find landmarks
wScr, hScr = autopy.screen.size()
# print(wScr, hScr)
frameR = 120  # frame reduction
smoothening = 7

while True:
    success, img = cap.read()
    img = detector.findHands(img)
    lmList, bbox = detector.findPosition(img)

# 2. Get the tip of index and middle finger
    if len(lmList) != 0:
        x1, y1 = lmList[8][1:]  # coordinates of index finger
        x2, y2 = lmList[12][1:]  # coordinates of middle finger
        # print(x1, y1, x2, y2)

# 3. Check which fingers are up
        fingers= detector.fingersUp()
        # print(fingers)
        cv2.rectangle(img, (frameR, frameR), (wCam - frameR, hCam - frameR),
                      (255, 0, 255), 2)

# 4. Only index finger : Moving mode
        if fingers[1]==1 and fingers[2]==0:

            x3 = np.interp(x1, (frameR, wCam-frameR), (0, wScr))  # 5. Coordinates conversion
            y3 = np.interp(y1, (frameR, hCam-frameR), (0, hScr))
# 6. Smoothening values
            clocX = plocX + (x3 -plocX) / smoothening
            clocY = plocY + (y3 -plocY) / smoothening

# 7. Mouse move
            autopy.mouse.move(wScr-x3, y3)
            cv2.circle(img, (x1, y1), 10, (255, 0, 255), cv2.FILLED)
            plocX, plocY = clocX, clocY

# 8. Both index and middle fingers are up : Clicking q
        if fingers[1] == 1 and fingers[2] == 1:

            # 9. Find distance between fingers
            length, img, lineInfo = detector.findDistance(8, 12, img)
            print(length)
            # 10. Click mouse if distance is short
            if length<30:
                cv2.circle(img, (lineInfo[4], lineInfo[5]), 10, (0,255,255), cv2.FILLED)

                autopy.mouse.click()

    cTime = time.time()            # 11. Frame rate
    fps = 1 / (cTime - pTime)
    pTime = cTime
    cv2.putText(img, f'FPS: {int(fps)}', (40, 40),
                cv2.FONT_HERSHEY_COMPLEX, 1, (255, 45, 0), 2)
    cv2.imshow("Image", img)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()


